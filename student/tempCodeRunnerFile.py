    ...
    while True:
        ...
        for face_location, face_encoding in zip(face_locations, face_encodings):
            ...
            if matches[best_match_index]:
                ...
                cursor = connection.cursor()
                cursor.execute("SELECT first_name, id FROM students WHERE id = %s", (person_id,))
                person_name, student_id = cursor.fetchone()
                cursor.close()
                
                # Draw rectangle around the face
                cv2.rectangle(frame, (left, top), (right, bottom), (0, 255, 0), 2)
                
                # Display name and ID on the camera
                cv2.putText(frame, f"Name: {person_name}", (left, top - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(frame, f"ID: {student_id}", (left, top - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
